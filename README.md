...
**功能：** 配额总览面板、多账号管理、OAuth 添加账号、首次本地账号自动检测与绑定、导入当前 `auth.json` 后立即刷新、跨窗口账号同步、Codex App 自动重启、状态栏监控、详情面板、中英文界面切换、扩展内语言覆盖设置。

**语言：** 跟随 VS Code 语言设置，当前支持简体中文、English。
...
- 也可以在扩展设置中单独指定本扩展使用中文或 English，不影响 VS Code 其他界面
...
目前暂未上架 VS Code 插件市场，当前仅支持通过 `.vsix` 文件手动安装。
